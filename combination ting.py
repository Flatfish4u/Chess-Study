...
# Ensure the file has a .pgn extension
...
def process_games(games, group_label, chunk_size=10000):
    """
    Processes a list of chess games and yields move-level data in chunks.
    """
    all_moves = []
    for i, game in enumerate(tqdm(games, desc=f'Processing {group_label} games')):
...
def fetch_lichess_games(username, max_games=1500):
    """
...
def main():
    ...
    for username in adhd_usernames:
        ...
        user_games = fetch_lichess_games(username, max_games=1500)  # Set to maximum allowed by Lichess API
        print(f"Fetched {len(user_games)} games for {username}.")
        if user_games:
            adhd_games.extend(user_games)
        else:
            print(f"No games found for {username}. Skipping.")

    print(f"Total ADHD games fetched: {len(adhd_games)}")
    if adhd_games:
        print("Processing ADHD players' games...")
        # Collect all chunks into a list and concatenate
        adhd_moves_chunks = list(process_games(adhd_games, group_label='ADHD'))
        if adhd_moves_chunks:
            adhd_moves_df = pd.concat(adhd_moves_chunks, ignore_index=True)
            print(f"ADHD moves DataFrame created with {len(adhd_moves_df)} moves.")
        else:
            print("No valid moves extracted for ADHD players. Exiting.")
            return
    else:
        print("No ADHD games fetched. Exiting.")
        return

    # ----------------------- 2. Fetch and Process General Population Games -----------------------

    print("Fetching general population games...")
    # Ensure the PGN file path is correct and points to a .pgn file
    if not os.path.exists(GENERAL_PGN_FILE_PATH):
        print(f"General population PGN file not found at {GENERAL_PGN_FILE_PATH}. Please check the path.")
        return

    print("Processing general population games...")
    general_games = process_pgn_file(GENERAL_PGN_FILE_PATH, max_games=10000)  # Limit to 10,000 games

    if not general_games:
        print("No games fetched for the General population. Exiting.")
        return

    print(f"Fetched {len(general_games)} general population games.")
    print("Processing general population games...")
    # Collect all chunks into a list and concatenate
    general_moves_chunks = list(process_games(general_games, group_label='General'))
    if general_moves_chunks:
        general_moves_df = pd.concat(general_moves_chunks, ignore_index=True)
        print(f"General moves DataFrame created with {len(general_moves_df)} moves.")
    else:
        print("No valid moves extracted for the General population. Exiting.")
        return
    ...
    all_moves_df['MoveNumber'] = all_moves_df['MoveNumber'].astype(np.int16)
    all_moves_df['Player'] = all_moves_df['Player'].astype('category')
    all_moves_df['Group'] = all_moves_df['Group'].astype('category')
    all_moves_df['ErrorCategory'] = all_moves_df['ErrorCategory'].astype('category')

    # Verify that both groups are present
    group_counts = all_moves_df['Group'].value_counts()
    print("\nGroup Distribution:")
    print(group_counts)
    if len(group_counts) < 2:
        print("Only one group detected in the dataset. Please check data processing.")
        return
    ...
    def perform_statistical_test(var, data, test_type='independent_t'):
        ...
        try:
            ...
            print(f"Shapiro-Wilk test failed for variable '{var}': {e}")
    ...
    def perform_chi_squared_test(category_var, data):
        ...
        try:
            ...
            print(f"Chi-Squared test failed for variable '{category_var}': {e}")
            return
    ...
    def plot_performance_under_time_pressure(df):
        ...
        if test_results:
            ...
            # Assuming the variable is 'EvalChange' under time pressure
            # We'll annotate significance in the statistical test results section
        ...
        sns.violinplot(x='Group', y='EvalChange', hue='Group', data=df_pressure, palette='Set2', inner='quartile', legend=False)
    ...
    def plot_accuracy_vs_time(df):
        ...
        sns.lineplot(x='TimeSpent', y='EvalChange', hue='Group', data=df, errorbar=None, estimator='mean', palette='Set1')
        ...
        # Regression plot
        sns.lmplot(x='TimeSpent', y='EvalChange', hue='Group', data=df, height=6, aspect=1.5, palette='Set1', scatter_kws={'alpha':0.2, 's':10}, errorbar=None)
    ...
    def plot_error_rate(df):
        ...
        total_counts = df.groupby('Group', observed=True)['ErrorCategory'].count().reset_index(name='Total')
    ...
    # results_df.to_csv('/Users/benjaminrosales/.spyder-py3/statistical_test_results.csv', index=False)
